// ====== Merged JS (profile.js + main.js) ======
// Each file runs only on pages where its DOM roots exist.
...
(function () {
  ...
  try {
    ...
    const HEIGHT_RANGE = { min: 150, max: 200 };
    const WEIGHT_RANGE = { min: 40, max: 130 };
    ...
    function createBrush(containerId, min, max, defaultMin, defaultMax, unit, onBrush) {
      ...
      // Use 30 for weight (40, 70, 100, 130), 10 for height (150..200)
      const step = (containerId === 'weight-range') ? 15 : 10;
      const axis = d3.axisBottom(x)
        .tickValues(d3.range(min, max + 1e-6, step))
        .tickFormat(d => `${Math.round(d)}${unit}`);
    ...
    d3.select('#continue-btn').on('click', function () {
      // Base guard (quick range check on the state values)
      ...
      // Full validation based on mode and inputs
    ...
    // === Restore selections when coming back from Explorer ===
  ...
  if (fromMain) {
    ...
    // Clear the flag so a regular refresh returns to Exact defaults
    localStorage.removeItem('from_main');
  }

  // Ensure exact defaults are visible if not overridden
  d3.select('#height-exact').property('value', state.height); // 170
  d3.select('#weight-exact').property('value', state.weight); // 70
  d3.select('#height-display').text(`${state.height} cm`);
  d3.select('#weight-display').text(`${state.weight} kg`);
...
(function () {
  ...
  if (!hasMain) return;
  try {
    // Optional: fallback to solid colors instead of patterns if images missing
    // If your original main.js already defines these, this override is harmless.
    ...
    function createMiniAvatar(svg, gender, height, weight) {
      ...
      const heightNorm = (height - 150) / (200 - 150);
      const weightNorm = (weight - 40) / (130 - 40);
      const heightScale = 0.85 + (heightNorm * 0.3);
      const widthScale = 0.85 + (weightNorm * 0.3);
    ...
    function renderDonut() {
      ...
      // --- (Labels) ---
      // --- (Outside labels + leader lines) ---
      const labelRadius = donutOuter + 80; // where text sits
      const bendRadius = donutOuter + 60; // elbow/bend of the polyline
      ...
      leaders.join(
        enter => enter.append("path")
          ...
          .attr("d", d => {
            // start at slice edge, bend near outer ring, end at label line
            ...
            // snap horizontal end to left/right for clean “branch out”
      ...
      labels.join(
        enter => enter.append("text")
          ...
          .attr("transform", d => {
            ...
            p[0] = (midAngle(d) < Math.PI) ? labelRadius : -labelRadius; // snap X
      ...
      try {
        ...
        if (userGender && userHeight && userWeight) {
          ...
          d3.select('#edit-profile-btn').on('click', () => {
            // Add a flag so Profile knows you actually came from the Explorer
            localStorage.setItem('from_main', '1');

            // Save gender (already shown in the profile card)
            localStorage.setItem('gender', (state.gender || 'All').toLowerCase());

            // Persist current filter ranges as the user's selection
            localStorage.setItem('height_mode', 'range');
            localStorage.setItem('weight_mode', 'range');

            // Height on main page is in meters — convert to cm for the profile page UI
            ...
            // Optional convenience values (averages) if you ever need exact
